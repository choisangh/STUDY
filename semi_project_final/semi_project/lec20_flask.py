...
def form_submit_get():
    ...
    return render_template("res.html", res_obj=my, f_info = f_info)     #render_template("index.html", MY_MSG="ok")
...
def chart_data(ent, select_date = None):
    ...
    # 처음 기업명 검색하고 들어온 경우 차트 기간을 얼마나 설정할지? : 일주일 - 상현님
        ...
        e_date = datetime.now()
        s_date = e_date - timedelta(days=20)
        print(f'e_date: {e_date}, s_date: {s_date}')
...
# 2022-02-19 KMS
# 코스피, 코스닥, 나스탁, 다우, S&p500 전날 현재 가격가져오는 함수.
def finance_data(stock_code):
    stock_list = [stock_code, '^KS11','^KQ11', '^IXIC', '^GSPC', '^DJI']
    res_list = {}
